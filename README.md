...
| /detect.jpg | GET | Captures one frame, runs YOLO11 object detection, draws bounding boxes, and returns the annotated jpeg image. |
...
This project serves on-demand snapshots (`/pic`) and YOLO-annotated snapshots (`/detect.jpg`), so each request gets one frame.

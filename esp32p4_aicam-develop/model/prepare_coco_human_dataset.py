...
def sample_image_infos(coco: dict, person_id: int, class_label: str, sample_count: int, rng: random.Random) -> list[dict]:
    ...
    for annotation in coco["annotations"]:
        ...
        image_to_person_boxes.setdefault(image_id, []).append(bbox)

    filtered_human_ids = []

    for image_id, boxes in image_to_person_boxes.items():
        ...
        for bbox in boxes:
            x, y, w, h = bbox
            person_area_ratio = float(w * h) / image_area

            if person_area_ratio > max_ratio:
...
# 가장 큰 사람 기준으로 판단
    ...
    if len(candidate_ids) < sample_count:
        raise RuntimeError(f"Requested {sample_count} {class_label} images, but only {len(candidate_ids)} available after filtering.")

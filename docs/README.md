...
P4는 SNTP를 실행하지 않습니다. 기본 구성에서는 노트북 Python 수신기가 같은
USB COM으로 ESP32-CAM Mesh 서버에 주는 절대시각이 기준입니다. 시간 표본은
서버에서 BLE Mesh
...
```text
Laptop OS clock → 28B USB UART update → ESP32-CAM server
  → Mesh CA/CB
  → C6
...
최초로 승인한 OPEN 수신시각을 `SERVER_TIME_RX_ESTIMATE`로 고정합니다. 서버가
노트북 시각을 아직 받지 못했거나 5분 동안 유효한 갱신을 받지 못했다면 시간은 0이고
`SERVER_TIME_UNKNOWN`입니다.
...
빌드하고, 노트북 시각을 console UART로 받습니다. 서버 firmware는
ESP32-CAM의 카메라 주변장치와 GPIO 또는 Wi-Fi를 사용하지 않습니다.
...
- 노트북 기준시각을 C6의 Mesh TIME 요청에 응답
...
`latest.jpg`/`latest.json`으로 저장하는 동시에, 반대 방향으로 노트북 Unix ms를
실행 직후와 1분마다 전송합니다. 이미지 송수신 중인 주기는 건너뜁니다. 별도
UART는 필요 없으며 `idf.py monitor`와
동시에 실행하면 안 됩니다.
...
야외 기본 구성은 Wi-Fi가 필요 없습니다. 선택형 Wi-Fi/SNTP를 쓰려면 serial
time을 끄고 Wi-Fi/SNTP를 켭니다. 펌웨어 HTTP 이미지 출력을 사용할 때는 serial
image도 끄고 `CONFIG_SERVER_HTTP_ENABLE=y`로 설정합니다. 비동기 저장·업로드
코드는 callback이 반환되기 전에 JPEG를 자체 복사해야 합니다.
...
시간 경로가 준비되면 서버에는 `laptop clock synchronized`, P4에는
